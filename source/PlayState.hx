...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(SONG.song.toLowerCase() == 'crucify')
		{
			...
			hitit.frames = Paths.getSparrowAtlas('effectShit/FUCKINGHITITBITCH');
			hitit.animation.addByPrefix('bump', 'fuckinHITIT', 24);
			hitit.animation.play('bump');
			hitit.antialiasing = true;
			hitit.cameras = [camHUD];
			add(hitit);
			hitit.visible = false;
	...
	function startSong():Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			var songName = new FlxText(0,songPosBG.y,0,SONG.song + ' - ' + CoolUtil.difficultyArray[storyDifficulty], 16);
			if (FlxG.save.data.downscroll)
				songName.y -= 3;
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (storyDifficulty == 0 || storyDifficulty == 4)
						difficulty = '-easy';
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			remove(scanlines); add(scanlines);
	...
	override function beatHit()
	{
		...
		if (curBeat % gfSpeed == 0 && boyfriend.animation.curAnim.name != 'hey')
		{
			gf.dance();
		}

		if (!boyfriend.animation.curAnim.name.startsWith("sing") && boyfriend.animation.curAnim.name != 'hey' || boyfriend.animation.curAnim.name == 'hey' && boyfriend.animation.finished)
		{
			boyfriend.playAnim('idle');
		}
